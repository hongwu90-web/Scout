...
import { itemAPI, focusFeedAPI, type Feed, type Item, type ListItemsParams } from "@/lib/api";
import {
  ...
  list: (filters: ItemFilters, pageSize: number) => {
    ...
    return infiniteQueryOptions({
      ...
      queryFn: async ({ pageParam }) => {
        const params = buildListItemsParams(normalizedFilters, pageParam, pageSize);
        if (normalizedFilters.focusFeedId) {
          return focusFeedAPI.listItems(normalizedFilters.focusFeedId, params);
        }
        return itemAPI.list(params);
      },
...
export function useItems(filters: ItemFilters, enabled = true) {
  ...
  return useInfiniteQuery({
    ...itemQueries.list(filters, articlePageSize),
    enabled,
...
function applyOptimisticItemReadState(
  ...
  for (const id of ids) {
    const cachedItem = qc.getQueryData<Item>(queryKeys.items.detail(id));
    if (cachedItem && !updatedItemsById.has(id)) {
      if (cachedItem.unread !== targetUnread) {
        const delta = targetUnread ? 1 : -1;
        feedDeltaMap.set(
          cachedItem.feed_id,
          (feedDeltaMap.get(cachedItem.feed_id) ?? 0) + delta,
        );
      }
    }
  ...
  return useMutation({
    ...
    onMutate: async (ids) => {
      await Promise.all([
        ...
        qc.cancelQueries({ queryKey: queryKeys.focusFeeds.all }),
      ]);
    ...
    onSettled: async () => {
      await Promise.all([
        ...
        qc.refetchQueries({ queryKey: queryKeys.feeds.list() }),
        qc.refetchQueries({ queryKey: queryKeys.focusFeeds.list() }),
      ]);
    },
  });
}
...
export function useMarkItemsReadByDate() {
  const qc = useQueryClient();

  return useMutation({
    mutationFn: async (data: { feed_id?: number; group_id?: number; before_date?: number }) => {
      await itemAPI.markReadByDate(data);
    },
    onSuccess: async () => {
      await Promise.all([
        qc.invalidateQueries({ queryKey: queryKeys.items.all }),
        ...
        qc.refetchQueries({ queryKey: queryKeys.feeds.list() }),
        qc.refetchQueries({ queryKey: queryKeys.focusFeeds.list() }),
        qc.refetchQueries({ queryKey: queryKeys.items.lists() }),
      ]);
    },
  });
}

export function usePurgeReadItems() {
  const qc = useQueryClient();

  return useMutation({
    mutationFn: async (data: { feed_id?: number; group_id?: number }) => {
      await itemAPI.purgeRead(data);
    },
    onSuccess: async () => {
      await Promise.all([
        qc.invalidateQueries({ queryKey: queryKeys.items.all }),
...
export function useTranslateItemTitle() {
  const qc = useQueryClient();

  return useMutation({
    mutationFn: async (itemId: number) => {
      return await itemAPI.translate(itemId);
    },
    onSuccess: (data, itemId) => {
      if (!data?.translated_title) return;

      // Update individual item detail in query cache
      qc.setQueryData<Item | undefined>(
      ...
      // Update item in infinite query lists
      qc.setQueriesData<ItemsInfiniteData>(
        { queryKey: queryKeys.items.lists() },
        (old) => {
          ...
          return {
            ...
            pages: old.pages.map((page) => ({
              ...
              data: page.data.map((item) =>
                item.id === itemId
                  ? { ...item, translated_title: data.translated_title }
                  : item,
              ),
            })),
          };
