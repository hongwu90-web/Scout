...
import { useFeedOrderStore } from "@/store";
...
export function useFeedLookup() {
  const { data: rawFeeds = [], isLoading } = useFeeds();
  const feedOrder = useFeedOrderStore((state) => state.feedOrder);
  ...
  const feeds = useMemo(() => {
    ...
    feedOrder.forEach((id, idx) => orderMap.set(id, idx));

    return [...rawFeeds].sort((a, b) => {
      const idxA = orderMap.has(a.id) ? orderMap.get(a.id)! : 999999 + a.id;
      const idxB = orderMap.has(b.id) ? orderMap.get(b.id)! : 999999 + b.id;
      return idxA - idxB;
    });
  }, [rawFeeds, feedOrder]);
