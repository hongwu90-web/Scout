...
export function useDeleteAllBookmarks() {
  const qc = useQueryClient();

  return useMutation({
    mutationFn: async (data?: { feed_id?: number; group_id?: number }) => {
      await bookmarkAPI.deleteAll(data);
    },
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: queryKeys.bookmarks.all });
      await qc.refetchQueries({ queryKey: queryKeys.bookmarks.all });
    },
  });
}

export function useBatchCreateBookmarks() {
  const qc = useQueryClient();

  return useMutation({
    mutationFn: async (itemIds: number[]) => {
      await bookmarkAPI.batchCreate({ item_ids: itemIds });
    },
    onSettled: () => {
      qc.invalidateQueries({ queryKey: queryKeys.bookmarks.all });
    },
...
export function useBatchDeleteBookmarks() {
  ...
  return useMutation({
    mutationFn: async (itemIds: number[]) => {
      await bookmarkAPI.batchDelete({ item_ids: itemIds });
    },
    onSettled: () => {
      qc.invalidateQueries({ queryKey: queryKeys.bookmarks.all });
    },
