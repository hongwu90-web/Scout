...
import { generateOPML, generateFeedsJSON, generateFeedsCSV, downloadFile } from "@/lib/opml";
...
function FeedsPage() {
  ...
  const handleExport = async (format: "opml" | "json" | "csv" = "opml") => {
    setIsExporting(true);
    try {
      ...
      if (format === "opml") {
        content = generateOPML(groupsData, feedsData);
        filename = "scout-subscriptions.opml";
        mimeType = "application/xml;charset=utf-8;";
      } else if (format === "json") {
        content = generateFeedsJSON(groupsData, feedsData);
        filename = "scout-subscriptions.json";
        mimeType = "application/json;charset=utf-8;";
      } else if (format === "csv") {
        content = generateFeedsCSV(groupsData, feedsData);
        filename = "scout-subscriptions.csv";
        mimeType = "text/csv;charset=utf-8;";
      }

      downloadFile(content, filename, mimeType);
      toast.success(`Exported ${feedsData.length} feeds (${filename})`);
    } catch (error) {
          ...
          <div className="flex items-center gap-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-52 font-mono text-xs">
                <DropdownMenuItem onClick={() => handleExport("opml")} className="cursor-pointer">
                  <span className="font-bold mr-2 text-primary">[OPML]</span> Subscriptions (.opml)
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => handleExport("json")} className="cursor-pointer">
                  <span className="font-bold mr-2 text-primary">[JSON]</span> Full Backup (.json)
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => handleExport("csv")} className="cursor-pointer">
                  <span className="font-bold mr-2 text-primary">[CSV]</span> Spreadsheet (.csv)
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
