...
func (p noopPuller) RefreshFeed(ctx context.Context, userID, feedID int64) error { return nil }

func (noopPuller) RefreshAll(context.Context) (int, error) { return 0, nil }

func (noopPuller) GetCloudFeedSync() *pull.CloudFeedSync { return nil }
...
func TestFeverReadAndMarkFlows(t *testing.T) {
	...
	group, err := st.CreateGroup(1, "Tech")
	if err != nil {
		t.Fatalf("create group: %v", err)
	}
	feed, err := st.CreateFeed(1, group.ID, "Fusion Feed", "https://example.com/rss.xml", "https://example.com", "")
	if err != nil {
		t.Fatalf("create feed: %v", err)
	}
	item, err := st.CreateItem(1, feed.ID, "guid-1", "Entry 1", "https://example.com/entry-1", "<p>Hello</p>", 1700000000)
	if err != nil {
	...
	exists, err := st.BookmarkExists(1, item.Link)
	if err != nil {
	...
	updatedItem, err := st.GetItem(1, item.ID)
	if err != nil {
...
func TestFeverMarkSavedLinksExistingBookmarkToItem(t *testing.T) {
	...
	item, err := st.CreateItem(1, feed.ID, "guid-2", "Entry 2", "https://example.com/entry-2", "<p>Hello 2</p>", 1700000001)
	if err != nil {
		t.Fatalf("create item: %v", err)
	}

	if _, err := st.CreateBookmark(1, nil, nil, item.Link, "Old Snapshot", "<p>old</p>", item.PubDate, feed.Name); err != nil {
...
func TestFeverFeedsIncludesFeedsGroups(t *testing.T) {
	...
	if _, err := st.CreateFeed(1, group.ID, "Fusion Feed", "https://example.com/rss.xml", "https://example.com", ""); err != nil {
		t.Fatalf("create feed: %v", err)
	}
...
func TestFeverItemsWithMaxIDZeroReturnsRecentItems(t *testing.T) {
	...
	if _, err := st.CreateItem(1, feed.ID, "guid-3", "Entry 3", "https://example.com/entry-3", "<p>Hello 3</p>", 1700000002); err != nil {
		t.Fatalf("create item: %v", err)
	}
...
func TestFeverMarkFeedReadRespectsBefore(t *testing.T) {
	...
	oldItem, err := st.CreateItem(1, feed.ID, "guid-old", "Old", "https://example.com/old", "<p>old</p>", 100)
	if err != nil {
		t.Fatalf("create old item: %v", err)
	}
	newItem, err := st.CreateItem(1, feed.ID, "guid-new", "New", "https://example.com/new", "<p>new</p>", 200)
	if err != nil {
	...
	updatedOld, err := st.GetItem(1, oldItem.ID)
	if err != nil {
		t.Fatalf("get old item: %v", err)
	}
	updatedNew, err := st.GetItem(1, newItem.ID)
	if err != nil {
...
func TestFeverMarkReadSupportsCSVItemIDs(t *testing.T) {
	...
	item1, err := st.CreateItem(1, feed.ID, "guid-4", "Entry 4", "https://example.com/entry-4", "<p>Hello 4</p>", 1700000003)
	if err != nil {
		t.Fatalf("create item 1: %v", err)
	}
	item2, err := st.CreateItem(1, feed.ID, "guid-5", "Entry 5", "https://example.com/entry-5", "<p>Hello 5</p>", 1700000004)
	if err != nil {
	...
	updated1, err := st.GetItem(1, item1.ID)
	if err != nil {
		t.Fatalf("get item1: %v", err)
	}
	updated2, err := st.GetItem(1, item2.ID)
	if err != nil {
