...
func (h *Handler) listItems(c *gin.Context) {
	...
	items, err := h.store.ListItems(userID, params)
	if err != nil {
		internalError(c, err, "list items")
		return
	}

	total, err := h.store.CountItems(userID, params)
	if err != nil {
...
func (h *Handler) markItemsRead(c *gin.Context) {
	...
	if err := h.store.BatchUpdateItemsUnread(userID, req.IDs, false); err != nil {
...
func (h *Handler) markItemsUnread(c *gin.Context) {
	...
	if err := h.store.BatchUpdateItemsUnread(userID, req.IDs, true); err != nil {
...
type markItemsReadByDateRequest struct {
	FeedID        *int64 `json:"feed_id"`
	GroupID       *int64 `json:"group_id"`
	BeforePubDate *int64 `json:"before_date"`
}

func (h *Handler) markItemsReadByDate(c *gin.Context) {
	userID := getUserID(c)
	var req markItemsReadByDateRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		badRequestError(c, "invalid request")
		return
	}

	if err := h.store.MarkItemsReadByDate(userID, req.FeedID, req.GroupID, req.BeforePubDate); err != nil {
		internalError(c, err, "mark items read by date")
		return
	}
...
type purgeReadItemsRequest struct {
	FeedID  *int64 `json:"feed_id"`
	GroupID *int64 `json:"group_id"`
}

func (h *Handler) purgeReadItems(c *gin.Context) {
	userID := getUserID(c)
	var req purgeReadItemsRequest
