...
func (h *Handler) fever(c *gin.Context) {
	...
	apiKey := c.Request.Form.Get("api_key")
	var userID int64
	authenticated := false

	if h.store != nil && apiKey != "" {
		if user, err := h.store.GetUserByFeverKey(apiKey); err == nil {
			userID = user.ID
			authenticated = true
		}
	}
	if !authenticated && h.verifyFeverAPIKey(apiKey) {
		userID = 1
		authenticated = true
	}
	...
	markResult, msg, err := h.handleFeverMark(userID, c.Request.Form)
	if err != nil {
	...
	if markResult.IncludeUnreadItemIDs {
		ids, err := h.store.ListUnreadItemIDs(userID)
		if err != nil {
	...
	if markResult.IncludeSavedItemIDs {
		ids, err := h.store.ListSavedItemIDs(userID)
		if err != nil {
	...
	if hasFeverFlag(c.Request.Form, "groups") {
		groups, feedsGroups, err := h.buildFeverGroupsPayload(userID)
		if err != nil {
	...
	if hasFeverFlag(c.Request.Form, "feeds") {
		feeds, feedsGroups, err := h.buildFeverFeedsPayload(userID)
		if err != nil {
	...
	if hasFeverFlag(c.Request.Form, "favicons") {
		favicons, err := h.buildFeverFaviconsPayload(userID)
		if err != nil {
	...
	if hasFeverFlag(c.Request.Form, "items") {
		items, totalItems, err := h.buildFeverItemsPayload(userID, c.Request.Form)
		if err != nil {
...
func (h *Handler) handleFeverMark(userID int64, form url.Values) (feverMarkResult, string, error) {
	...
	case "item":
		...
		switch as {
		case "read":
			if err := h.store.BatchUpdateItemsUnread(userID, ids, false); err != nil {
				return feverMarkResult{}, "", err
			}
			return feverMarkResult{IncludeUnreadItemIDs: true}, "", nil
		case "unread":
			if err := h.store.BatchUpdateItemsUnread(userID, ids, true); err != nil {
				return feverMarkResult{}, "", err
			}
			return feverMarkResult{IncludeUnreadItemIDs: true}, "", nil
		case "saved":
			for _, id := range ids {
				if err := h.markItemSaved(userID, id); err != nil {
					return feverMarkResult{}, "", err
				}
			}
		...
		case "unsaved":
			for _, id := range ids {
				if err := h.markItemUnsaved(userID, id); err != nil {
					return feverMarkResult{}, "", err
				}
			}
			return feverMarkResult{IncludeSavedItemIDs: true}, "", nil
	...
	case "feed":
		...
		if err := h.store.MarkFeedAsReadBefore(userID, id, before); err != nil {
			return feverMarkResult{}, "", err
		}
		return feverMarkResult{IncludeUnreadItemIDs: true}, "", nil
	case "group":
		...
		if id == 0 {
			if err := h.store.MarkAllAsReadBefore(userID, before); err != nil {
				return feverMarkResult{}, "", err
			}
			return feverMarkResult{IncludeUnreadItemIDs: true}, "", nil
		}
		if err := h.store.MarkGroupAsReadBefore(userID, id, before); err != nil {
			return feverMarkResult{}, "", err
		}
		return feverMarkResult{IncludeUnreadItemIDs: true}, "", nil
	default:
...
func (h *Handler) markItemSaved(userID int64, itemID int64) error {
	item, err := h.store.GetItem(userID, itemID)
	if err != nil {
	...
	exists, err := h.store.BookmarkExists(userID, item.Link)
	if err != nil {
		return err
	}
	if exists {
		return h.store.UpdateBookmarkItemIDByLink(userID, item.ID, item.Link)
	}
	...
	_, err = h.store.CreateBookmark(userID, &item.ID, &item.FeedID, item.Link, item.Title, item.Content, item.PubDate, feed.Name)
	return err
}

func (h *Handler) markItemUnsaved(userID int64, itemID int64) error {
	item, err := h.store.GetItem(userID, itemID)
	if err != nil {
		if errors.Is(err, store.ErrNotFound) {
			return nil
	...
	return h.store.DeleteBookmarkByLink(userID, item.Link)
}

func (h *Handler) buildFeverGroupsPayload(userID int64) ([]feverGroup, []feverFeedsGroup, error) {
...
func (h *Handler) buildFeverFeedsPayload(userID int64) ([]feverFeed, []feverFeedsGroup, error) {
	feeds, err := h.store.ListFeeds(userID)
	if err != nil {
		return nil, nil, err
	}
...
func (h *Handler) buildFeverFaviconsPayload(userID int64) ([]feverFavicon, error) {
	feeds, err := h.store.ListFeeds(userID)
	if err != nil {
		return nil, err
	}
...
func (h *Handler) buildFeverItemsPayload(userID int64, form url.Values) ([]feverItem, int, error) {
	params, err := parseListFeverItemsParams(form)
	if err != nil {
		return nil, 0, err
	}

	items, err := h.store.ListFeverItems(userID, params)
	if err != nil {
		return nil, 0, err
	}

	totalItems, err := h.store.CountItems(userID, store.ListItemsParams{})
	if err != nil {
		return nil, 0, err
	}

	savedIDs, err := h.store.ListSavedItemIDs(userID)
	if err != nil {
		return nil, 0, err
	}
