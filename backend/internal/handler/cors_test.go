...
func TestCORSMiddleware(t *testing.T) {
	tests := []struct {
		...
		{
			name:            "allows configured origin with credentials",
		...
		{
			name:            "allows any origin when not configured but without credentials",
			method:          http.MethodGet,
		...
		{
			name:            "allows localhost origin with credentials when not configured",
			method:          http.MethodGet,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if gotCred := w.Header().Get("Access-Control-Allow-Credentials"); gotCred != tt.wantCredentials {
				t.Fatalf("expected allow-credentials header %q, got %q", tt.wantCredentials, gotCred)
			}
		})
	}
}
