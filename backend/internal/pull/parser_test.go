...
func TestFetchAndParseSendsConditionalHeadersAndHandles304(t *testing.T) {
	...
	result, err := FetchAndParse(context.Background(), feed, feed.Link, 5*time.Second, true)
	if err != nil {
...
func TestFetchAndParseSyntheticFeed(t *testing.T) {
...
<body>
  <article>
    <h2><a href="/story/1">First Big News</a></h2>
    <p>Details about the story</p>
  </article>
  <article>
    <h2><a href="/story/2">Second Big News</a></h2>
    <p>More details</p>
  </article>
</body>
</html>`))
	...
	syntheticTarget := "/api/feeds/synthetic?url=" + url.QueryEscape(ts.URL) + "&selector=article"
	feed := &model.Feed{
		Link: syntheticTarget,
	}

	result, err := FetchAndParse(context.Background(), feed, syntheticTarget, 5*time.Second, true)
	if err != nil {
		t.Fatalf("FetchAndParse synthetic feed failed: %v", err)
	}
	...
	if result.Items[0].Title != "First Big News" {
		t.Errorf("unexpected item 0 title: %s", result.Items[0].Title)
	}
}
