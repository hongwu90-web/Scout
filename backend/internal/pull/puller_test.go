...
<item><guid>g1</guid><title>Item</title><link>https://example.com/1</link></item>
	...
	feed, err := st.CreateFeed(1, 1, "Feed A", server.URL, "", "")
	if err != nil {
	...
	if err := p.RefreshFeed(ctx, 1, feed.ID); err != nil {
	...
	updatedFeed, err := st.GetFeed(1, feed.ID)
	if err != nil {
...
<item><guid>%s</guid><title>Item</title><link>https://example.com%s</link></item>
	...
	if _, err := st.CreateFeed(1, 1, "Feed A", server.URL+"/a", "", ""); err != nil {
		t.Fatalf("create feed A: %v", err)
	}
	if _, err := st.CreateFeed(1, 1, "Feed B", server.URL+"/b", "", ""); err != nil {
