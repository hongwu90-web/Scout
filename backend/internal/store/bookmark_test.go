...
func TestListBookmarks(t *testing.T) {
	...
	// Test empty list
	bookmarks, err := store.ListBookmarks(1, ListBookmarksParams{Limit: 10})
	if err != nil {
	...
	t.Run("pagination with limit", func(t *testing.T) {
		bookmarks, err := store.ListBookmarks(1, ListBookmarksParams{Limit: 2})
		if err != nil {
			t.Fatalf("ListBookmarks() failed: %v", err)
		}
	...
	t.Run("pagination with cursor", func(t *testing.T) {
		...
		page1, err := store.ListBookmarks(1, ListBookmarksParams{Limit: 2})
		if err != nil {
			t.Fatalf("ListBookmarks() failed: %v", err)
		}
		if len(page1) != 2 {
		...
		page2, err := store.ListBookmarks(1, ListBookmarksParams{
			Limit:           2,
		...
		page3, err := store.ListBookmarks(1, ListBookmarksParams{
			Limit:           2,
			BeforeCreatedAt: &last.CreatedAt,
			BeforeID:        &last.ID,
		})
...
func TestListBookmarksFilters(t *testing.T) {
	...
	if err := store.UpdateItemUnread(1, itemB.ID, false); err != nil {
	...
	t.Run("filter by feed_id", func(t *testing.T) {
		got, err := store.ListBookmarks(1, ListBookmarksParams{FeedID: &feedA.ID, Limit: 10})
		if err != nil {
			t.Fatalf("ListBookmarks() failed: %v", err)
		}
		...
		count, err := store.CountBookmarks(1, ListBookmarksParams{FeedID: &feedA.ID})
		if err != nil {
	...
	t.Run("filter by group_id joins feeds", func(t *testing.T) {
		got, err := store.ListBookmarks(1, ListBookmarksParams{GroupID: &group1.ID, Limit: 10})
		if err != nil {
			t.Fatalf("ListBookmarks() failed: %v", err)
		}
		...
		count, err := store.CountBookmarks(1, ListBookmarksParams{GroupID: &group1.ID})
		if err != nil {
			t.Fatalf("CountBookmarks() failed: %v", err)
		}
		if count != 2 {
	...
	t.Run("unread mirrors linked item", func(t *testing.T) {
		...
		gotB, err := store.ListBookmarks(1, ListBookmarksParams{FeedID: &feedB.ID, Limit: 10})
		if err != nil {
			t.Fatalf("ListBookmarks() failed: %v", err)
		}
		if gotB[0].Unread {
	...
	t.Run("orphan bookmark has nil feed_id and reads as read", func(t *testing.T) {
		got, err := store.ListBookmarks(1, ListBookmarksParams{Limit: 10})
		if err != nil {
			t.Fatalf("ListBookmarks() failed: %v", err)
		}
	...
	t.Run("pagination applies after feed_id filter", func(t *testing.T) {
		...
		total, err := store.CountBookmarks(1, ListBookmarksParams{FeedID: &pagFeed.ID})
		if err != nil {
			t.Fatalf("CountBookmarks() failed: %v", err)
		}
		if total != n {
		...
		page1, err := store.ListBookmarks(1, ListBookmarksParams{FeedID: &pagFeed.ID, Limit: 2})
		if err != nil {
...
func TestGetBookmark(t *testing.T) {
	...
	bookmark, err := store.GetBookmark(1, created.ID)
	if err != nil {
	...
	_, err = store.GetBookmark(1, 99999)
...
func TestCreateBookmark(t *testing.T) {
	...
	t.Run("unique constraint on link", func(t *testing.T) {
		...
		_, err := store.CreateBookmark(1, nil, nil, link, "Bookmark 2", "Content", 123, "Feed")
		if err == nil {
...
func TestDeleteBookmark(t *testing.T) {
	...
	// Delete bookmark
	if err := store.DeleteBookmark(1, bookmark.ID); err != nil {
		t.Fatalf("DeleteBookmark() failed: %v", err)
	}

	// Verify deletion
	_, err := store.GetBookmark(1, bookmark.ID)
	if !errors.Is(err, ErrNotFound) {
...
func TestBookmarkExists(t *testing.T) {
	...
	exists, err := store.BookmarkExists(1, link)
	if err != nil {
	...
	exists, err = store.BookmarkExists(1, "https://example.com/nonexistent")
	if err != nil {
		t.Fatalf("BookmarkExists() failed: %v", err)
	}
