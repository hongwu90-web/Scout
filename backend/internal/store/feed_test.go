...
func TestGetFeed(t *testing.T) {
	...
	// Get existing feed
	feed, err := store.GetFeed(1, created.ID)
	if err != nil {
...
func TestCreateFeed(t *testing.T) {
	...
	_, err := store.CreateFeed(1, group.ID, "Duplicate", link, siteURL, "")
	if err == nil {
...
func TestUpdateFeed(t *testing.T) {
	...
	if err := store.UpdateFeed(1, feed.ID, params); err != nil {
		t.Fatalf("UpdateFeed() failed: %v", err)
	}

	updated, err := store.GetFeed(1, feed.ID)
	if err != nil {
		t.Fatalf("GetFeed() failed: %v", err)
	}
	...
	if err := store.UpdateFeed(1, feed.ID, params2); err != nil {
		t.Fatalf("UpdateFeed() failed: %v", err)
	}

	updated2, err := store.GetFeed(1, feed.ID)
	if err != nil {
		t.Fatalf("GetFeed() failed: %v", err)
	}
...
func TestUpdateFeedLinkResetsFetchState(t *testing.T) {
	...
	if err := store.UpdateFeedFetchSuccess(1, feed.ID, UpdateFeedFetchSuccessParams{
		CheckedAt:    checkedAt,
		HTTPStatus:   200,
		ETag:         "etag-v1",
	...
	if err := store.UpdateFeed(1, feed.ID, UpdateFeedParams{Link: &newLink}); err != nil {
		t.Fatalf("UpdateFeed() failed: %v", err)
	}
...
func TestDeleteFeed(t *testing.T) {
	...
	if err := store.DeleteFeed(1, feed.ID); err != nil {
		t.Fatalf("DeleteFeed() failed: %v", err)
	}

	_, err := store.GetFeed(1, feed.ID)
	if !errors.Is(err, ErrNotFound) {
		t.Fatalf("expected ErrNotFound after deletion, got %v", err)
	}

	_, err = store.GetItem(1, item.ID)
	if !errors.Is(err, ErrNotFound) {
		t.Fatalf("expected ErrNotFound for item after feed deletion, got %v", err)
	}

	updatedBookmark, err := store.GetBookmark(1, bookmark.ID)
	if err != nil {
...
func TestUpdateFeedFetchSuccess(t *testing.T) {
	...
	if err := store.UpdateFeedFetchFailure(1, feed.ID, UpdateFeedFetchFailureParams{
		CheckedAt:       time.Now().Unix(),
		HTTPStatus:      500,
...
func TestUpdateFeedFetchFailure(t *testing.T) {
	...
	updated1, err := store.GetFeed(1, feed.ID)
	if err != nil {
		t.Fatalf("GetFeed() failed: %v", err)
	}
...
func TestUpdateFeedFetchFailureConcurrentIncrements(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		err1 = store.UpdateFeedFetchFailure(1, feed.ID, params)
	}()
	go func() {
		defer wg.Done()
		err2 = store.UpdateFeedFetchFailure(1, feed.ID, params)
	}()
	wg.Wait()
...
func TestUpdateFeedSiteURLIfEmpty(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := store.UpdateFeedSiteURLIfEmpty(1, feed.ID, firstSiteURL); err != nil {
			...
			if err := store.UpdateFeedSiteURLIfEmpty(1, feed.ID, secondSiteURL); err != nil {
...
func TestUpdateFeedNoParamsDoesNothing(t *testing.T) {
	...
	if err := store.UpdateFeed(1, feed.ID, UpdateFeedParams{}); err != nil {
		t.Fatalf("UpdateFeed() failed: %v", err)
	}
...
func TestUpdateFeedNotFound(t *testing.T) {
	...
	name := "Updated"
	err := store.UpdateFeed(1, 99999, UpdateFeedParams{Name: &name})
	if !errors.Is(err, ErrNotFound) {
...
func TestBatchCreateFeedsHandlesExistingAndInBatchDuplicates(t *testing.T) {
	...
	result, err := store.BatchCreateFeeds(1, []BatchCreateFeedsInput{
