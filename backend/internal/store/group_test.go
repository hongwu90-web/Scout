...
func TestGetGroup(t *testing.T) {
	...
	// Get existing group
	group, err := store.GetGroup(1, created.ID)
	if err != nil {
...
func TestCreateGroup(t *testing.T) {
	...
	name := "New Group"
	group, err := store.CreateGroup(1, name)
	if err != nil {
	...
	_, err = store.CreateGroup(1, name)
	if err == nil {
...
func TestUpdateGroup(t *testing.T) {
	...
	if err := store.UpdateGroup(1, group.ID, newName); err != nil {
		t.Fatalf("UpdateGroup() failed: %v", err)
	}

	// Verify update
	updated, err := store.GetGroup(1, group.ID)
	if err != nil {
		t.Fatalf("GetGroup() failed: %v", err)
	}
...
func TestDeleteGroup(t *testing.T) {
	...
	t.Run("delete normal group", func(t *testing.T) {
		...
		if err := store.DeleteGroup(1, group.ID); err != nil {
			t.Fatalf("DeleteGroup() failed: %v", err)
		}

		// Verify deletion
		_, err := store.GetGroup(1, group.ID)
		if !errors.Is(err, ErrNotFound) {
	...
	t.Run("cascade feeds to default group", func(t *testing.T) {
		...
		updatedFeed, err := store.GetFeed(1, feed.ID)
		if err != nil {
