...
func mustCreateFeed(t *testing.T, store *Store, groupID int64, name, link, siteURL, proxy string) *model.Feed {
	...
	feed, err := store.CreateFeed(1, groupID, name, link, siteURL, proxy)
	if err != nil {
...
func mustCreateItem(t *testing.T, store *Store, feedID int64, guid, title, link, content string, pubDate int64) *model.Item {
	...
	item, err := store.CreateItem(1, feedID, guid, title, link, content, pubDate)
	if err != nil {
...
func mustCreateBookmark(t *testing.T, store *Store, itemID *int64, feedID *int64, link, title, content string, pubDate int64, feedName string) *model.Bookmark {
	...
	bookmark, err := store.CreateBookmark(1, itemID, feedID, link, title, content, pubDate, feedName)
	if err != nil {
