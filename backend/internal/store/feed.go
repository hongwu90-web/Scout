...
func (s *Store) ListAllFeeds() ([]*model.Feed, error) {
	rows, err := s.db.Query(`
		SELECT f.id, f.user_id, f.group_id, f.name, f.link, f.site_url,
		...
		LEFT JOIN items i ON i.feed_id = f.id AND i.user_id = f.user_id
		GROUP BY f.id, f.user_id, f.group_id, f.name, f.link, f.site_url,
		         f.suspended, f.proxy, f.created_at, f.updated_at,
	...
	for rows.Next() {
		f := &model.Feed{}
		if err := rows.Scan(
			&f.ID, &f.UserID, &f.GroupID, &f.Name, &f.Link, &f.SiteURL,
			&f.Suspended, &f.Proxy, &f.CreatedAt, &f.UpdatedAt,
			&f.FetchState.ETag, &f.FetchState.LastModified, &f.FetchState.CacheControl,
			&f.FetchState.ExpiresAt, &f.FetchState.LastCheckedAt, &f.FetchState.NextCheckAt,
			&f.FetchState.LastHTTPStatus, &f.FetchState.RetryAfterUntil, &f.FetchState.LastSuccessAt,
			&f.FetchState.LastErrorAt, &f.FetchState.LastError, &f.FetchState.ConsecutiveFailures,
			&f.UnreadCount, &f.ItemCount,
		); err != nil {
			return nil, err
		}
...
func (s *Store) ListFeeds(userID int64) ([]*model.Feed, error) {
	rows, err := s.db.Query(`
		...
		FROM feeds f
		LEFT JOIN feed_fetch_state fs ON fs.feed_id = f.id
		LEFT JOIN items i ON i.feed_id = f.id AND i.user_id = :user_id
...
func (s *Store) GetFeed(userID int64, id int64) (*model.Feed, error) {
	f := &model.Feed{}
	var suspended int
	err := s.db.QueryRow(`
		SELECT f.id, f.group_id, f.name, f.link, f.site_url,
...
func (s *Store) CreateFeed(userID int64, groupID int64, name, link, siteURL, proxy string) (*model.Feed, error) {
	tx, err := s.db.Begin()
	if err != nil {
		return nil, err
	}
	...
	result, err := tx.Exec(`
		INSERT INTO feeds (user_id, group_id, name, link, site_url, proxy)
		VALUES (:user_id, :group_id, :name, :link, :site_url, :proxy)
	`, sql.Named("user_id", userID), sql.Named("group_id", groupID), sql.Named("name", name), sql.Named("link", link),
...
func (s *Store) SearchFeeds(userID int64, query string) ([]*SearchFeedResult, error) {
	rows, err := s.db.Query(`
...
func (s *Store) UpdateFeed(userID int64, id int64, params UpdateFeedParams) error {
	setClauses := []string{}
	...
	query := fmt.Sprintf("UPDATE feeds SET %s WHERE id = :id AND user_id = :user_id", strings.Join(setClauses, ", "))
...
func (s *Store) DeleteFeed(userID int64, id int64) error {
	tx, err := s.db.Begin()
	if err != nil {
		return err
	}
	...
	result, err := tx.Exec(`DELETE FROM feeds WHERE id = :id AND user_id = :user_id`, sql.Named("id", id), sql.Named("user_id", userID))
	if err != nil {
		return err
	}
	rows, err := result.RowsAffected()
...
func (s *Store) UpdateFeedFetchSuccess(userID int64, id int64, params UpdateFeedFetchSuccessParams) error {
	s.writeMu.Lock()
...
func (s *Store) UpdateFeedFetchFailure(userID int64, id int64, params UpdateFeedFetchFailureParams) error {
	s.writeMu.Lock()
	defer s.writeMu.Unlock()
...
func (s *Store) UpdateFeedSiteURLIfEmpty(userID int64, id int64, siteURL string) error {
	...
	_, err := s.db.Exec(`
		...
		WHERE id = :id AND user_id = :user_id AND (site_url IS NULL OR TRIM(site_url) = '')
	`, sql.Named("site_url", siteURL), sql.Named("id", id), sql.Named("user_id", userID))
	return err
}
...
func (s *Store) BatchCreateFeeds(userID int64, inputs []BatchCreateFeedsInput) (*BatchCreateFeedsResult, error) {
	...
	stmt, err := tx.Prepare(`
		INSERT INTO feeds (user_id, group_id, name, link, site_url, proxy)
		VALUES (:user_id, :group_id, :name, :link, :site_url, '')
