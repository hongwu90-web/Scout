...
type Config struct {
	...
	Host          string // Listening host (default: 127.0.0.1 for local isolation)
	Port          int
	...
	EnableRegistration bool     // Allow open registration of new users.
	...
	// Cloud Monitor Configuration (Cloudflare 24/7 autonomous scraper sync)
	CloudMonitorEnabled bool   // Enable sync with Cloudflare worker
	CloudMonitorURL     string // URL of Cloudflare worker
	CloudMonitorSecret  string // Secret key for Cloudflare worker sync
}
...
func Load() (*Config, error) {
	...
	allowEmptyPassword, err := getEnvBool("FUSION_ALLOW_EMPTY_PASSWORD", true)
	if err != nil {
		return nil, err
	}
	...
	enableRegistration, err := getEnvBool("FUSION_ENABLE_REGISTRATION", true)
	if err != nil {
		return nil, err
	}
	...
	return &Config{
		DBPath:             dbPath,
		Password:           password,
		Host:               getEnvString("FUSION_HOST", "127.0.0.1"),
		Port:               parsedPort,
		...
		CloudMonitorEnabled: getEnvString("CLOUD_MONITOR_ENABLED", "false") == "true",
		CloudMonitorURL:     strings.TrimRight(getEnvString("CLOUD_MONITOR_URL", ""), "/"),
		CloudMonitorSecret:  getEnvString("CLOUD_MONITOR_SECRET", ""),
	}, nil
}
...
func loadDotEnv() {
	...
	for _, path := range candidates {
		...
		for _, line := range strings.Split(string(data), "\n") {
			...
			// Do not override environment variables already set explicitly
